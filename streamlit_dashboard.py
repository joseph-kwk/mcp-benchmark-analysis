...
if "sim_ticks"        not in st.session_state: st.session_state.sim_ticks = 0
if "history"          not in st.session_state: st.session_state.history = []
if "trial_count"      not in st.session_state: st.session_state.trial_count = 0
if "last_server_resp" not in st.session_state: st.session_state.last_server_resp = None

TASKS = ["irrigation_check", "field_status", "weather_forecast",
...
with st.sidebar:
    ...
    task_choice    = st.selectbox("Task", TASKS)
...
st.caption(
    "Evaluating the Model Context Protocol for AI tool integration in smart farming  ·  "
...
with c3:
    if st.button("Fetch from Server", use_container_width=True,
                 disabled=not _SERVER_AVAILABLE, help="Calls get_field_status() on server.py"):
        s = time.perf_counter()
        ...
        for fid, resp in responses.items():
            if "moisture_pct" in resp:
                st.session_state.fields[_km[fid]]["moisture"] = float(resp["moisture_pct"])
...
for i, (fname, fdata) in enumerate(st.session_state.fields.items()):
    ...
    icon   = "🔴"             if m < 10 else ("🟡"            if m < 20 else "🟢")
    with fcols[i]:
        st.markdown(f"""
        <div class="field-card {css}">
            <div class="fc-name">{fname}</div>
            <div class="fc-moisture">{m:.0f}%</div>
...
with fa: st.markdown('<div class="flow-node">Sensor Data<br><small>Soil moisture</small></div>', unsafe_allow_html=True)
with fb: st.markdown("<div class='flow-arrow'>-></div>", unsafe_allow_html=True)
with fc: st.markdown('<div class="flow-node">FastMCP Server<br><small>activate_irrigation()<br>Key vault protected</small></div>', unsafe_allow_html=True)
with fd: st.markdown("<div class='flow-arrow'>-></div>", unsafe_allow_html=True)
with fe: st.markdown('<div class="flow-node">GPT-4o<br><small>Selects tool via<br>JSON-RPC 2.0</small></div>', unsafe_allow_html=True)
...
run_col, info_col = st.columns([1.2, 5])
with run_col:
    run_clicked = st.button("Run Benchmark", type="primary", use_container_width=True)
with info_col:
    st.caption(f"Task: **{task_choice}** · Trials: **{trials_per_run}** · Model: **GPT-4o**")

if run_clicked:
    progress = st.progress(0, text="Running trials...")
    for i in range(trials_per_run):
        seed = st.session_state.trial_count + i
        trad_r = MockLLMClient(LLMProvider.GPT4O, protocol="traditional", seed=seed).call(task_choice)
        mcp_r  = MockLLMClient(LLMProvider.GPT4O, protocol="mcp",         seed=seed).call(task_choice)
        st.session_state.history.append({
            ...
            "trad_success":    trad_r.success,
            "trad_tool":       trad_r.tool_called,
            ...
            "mcp_success":     mcp_r.success,
            "mcp_tool":        mcp_r.tool_called,
    ...
    pd.DataFrame(st.session_state.history).to_csv("benchmark/results/streamlit_results.csv", index=False)
    st.rerun()
...
lp, rp = st.columns(2)
with lp:
    css  = "result-card" + ("" if latest["trad_success"] else " fail")
    icon = "Pass" if latest["trad_success"] else "Fail"
    st.markdown(f"""
    <div class="{css}">
        [{icon}]  Tool: <b>{latest['trad_tool']}</b><br>
        Latency: <b>{latest['trad_latency_ms']:.0f} ms</b>  |  Tool exec: <b>{latest['trad_exec_ms']:.1f} ms</b><br>
...
with rp:
    css  = "result-card" + ("" if latest["mcp_success"] else " fail")
    icon = "Pass" if latest["mcp_success"] else "Fail"
    st.markdown(f"""
    <div class="{css}">
        [{icon}]  Tool: <b>{latest['mcp_tool']}</b><br>
        Latency: <b>{latest['mcp_latency_ms']:.0f} ms</b>  |  Tool exec: <b>{latest['mcp_exec_ms']:.1f} ms</b><br>
...
st.markdown(f'<div class="section-head">Section 3 — Aggregate ({len(df)} trials)</div>', unsafe_allow_html=True)
...
<div class="info-banner">
  ...
  Set <code>USE_REAL_APIS=true</code> in <code>.env</code> to replace with live GPT-4o measurements.
...
with c4b:
    ...
    st.altair_chart(
        alt.Chart(acc_df).mark_line(point=True, strokeWidth=2.5).encode(
            x=alt.X("trial:Q", title="Trial #"),
            y=alt.Y("Accuracy (%):Q", scale=alt.Scale(domain=[50,105])),
            color=alt.Color("Protocol:N", scale=COLORS, legend=alt.Legend(orient="top")),
...
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-head">Section 5 — Statistical Analysis and Interoperability</div>', unsafe_allow_html=True)
    ...
    if len(df) >= _MIN:
        try:
            ...
            s1.metric("Latency p-value", f"{_p:.4f}",
                      delta="sig." if _p<.05 else "not sig.", delta_color="normal" if _p<.05 else "off")
            ...
            s3.metric("Accuracy p-value", f"{_acc_p:.4f}",
                      delta="sig." if _acc_p<.05 else "not sig.", delta_color="normal" if _acc_p<.05 else "off")
            s4.metric("n trials", len(df))
            if _p < 0.05:
                st.success(f"Latency difference is statistically significant (p={_p:.4f}). Cohen's d = {_d:.3f}.")
            else:
                st.info(f"Not yet significant (p={_p:.4f}). Run more trials.")
        except ImportError:
...
with loc_col:
    ...
    st.caption("MCP exposes tools via JSON-RPC 2.0. GPT-4o, Gemini, and Claude all connect identically — zero code changes.")
